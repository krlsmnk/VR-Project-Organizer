...
namespace CAVS.ProjectOrganizer.Scenes.Testing.ToyCarTesting
{
    ...
    public class GameManager : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            {
                CarFactory.MakeToyCar(cars[i], cars[i].GetValue("id"), float.Parse(cars[i].GetValue("id")), (Vector3.right * (i% width)) + Vector3.forward * 2 * Mathf.Floor(i/ width), Quaternion.identity);
            }
        }
